...
```html
<!-- Wrap the images or canvas element with a block element (container) -->
<div>
...
```css
/* Limit images width to avoid overflow the container */
img {
...
// Upload cropped images to server if the browser supports `HTMLCanvasElement.toBlob`
